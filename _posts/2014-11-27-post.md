...
There have been some confusing claims about the immunity of Bayesian AB test to repeated testing. We can use the repeated testing framework introduced above to show otherwise. The plot below shows the probability of A winning and the expected run time as a function of lift for different testing intervals. To generate the plots, I used the same parameters as above, but lowered the cost to 0.001. We see the same pattern as in repeated hypothesis testing: you can trade off accuracy with sample size by changing the interval at which you test. It is by no means true that you do not reduce the chance of picking the better banner as you repeat the Bayesian test until hitting your stopping criterion.
