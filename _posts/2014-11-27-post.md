...
For the particular parameters I choose, the cost criterion has a uniformly higher chance of declaring A the better banner (both when it is true and when it is false). This is largely because it declares the winner to be unknown much less often and so is more likely to make mistakes when A is worse. The big win of the cost method comes from needing only 40% of the samples when the lift is less than 20%. The goal of the comparison is more to characterize the methods than to declare a winner. I suggest playing around with both methods to see which one is more desirable for a specific use case. You can find the ipython notebook I used to run these kinds of AB tests and generate the figures [here](https://github.com/ewulczyn/wmf/blob/master/self_terminating_ab/src/python/blog_experiments.ipynb)
